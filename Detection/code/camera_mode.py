...
class Camera(ttk.Frame):
        ...
        if messagebox.askyesno("Bắt đầu buổi học?", "Bạn có muốn bắt đầu một buổi học (session) mới không?\n(Kết quả sẽ được lưu vào CSDL)", parent=self.root):
            ...
            try:
                ...
                if new_session_id is None:
                    # LỖI CÓ THỂ XẢY RA Ở ĐÂY (NẾU BẠN CHƯA SỬA DB)
    ...
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # >>> [SỬA 4] HÀM GUI_LOOP (SỬA LỖI PAUSE) <<<
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    def gui_loop(self):
        ...
        try:
            ...
            # (Phần xử lý khi không có frame giữ nguyên)
            if frame is None:
                if not self.running:
                ...
                try:
                    ...
                    # [SỬA 4] Lấy trạng thái pause MỘT LẦN
                    ...
                    for i, student_id_db in enumerate(student_ids): 
                        ...
                        try:
                            ...
                            # [SỬA 4] Thêm kiểm tra 'if not is_paused'
                            if not is_paused:
                                # Chỉ cập nhật điểm/timer nếu video KHÔNG bị pause
                    ...
                    # (Phần cập nhật Treeview và ngắt dòng giữ nguyên)
                    for item in self.info_tree.get_children():
                        self.info_tree.delete(item)
    ...
    def finalize_session(self):
        ...
        if self.current_session_id is None:
            # print("Không có session nào đang hoạt động để kết thúc.")
        ...
        # 4. Cập nhật bảng 'seasion' (end_time)
        try:
        ...
        messagebox.showinfo("Đang xử lý", f"Đang lưu kết quả session {session_to_finalize} cho {len(students_to_finalize)} học sinh...", parent=self.root)

        # 6. Lặp qua từng học sinh và lưu
        for student_id in students_to_finalize:
            ...
            try:
                ...
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                # >>> [SỬA 2] GỌI AI ĐỂ TÓM TẮT LOGS <<<
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                note = "Không có ghi nhận chi tiết."
                try:
                    # 1. Vẫn lấy logs thô từ manager
                    # (Yêu cầu focus_manager.py phải có hàm này)
                    logs = self.focus_manager.get_student_full_logs(manager_id)
                    
                    if logs:
                        # 2. Gửi logs cho AI tóm tắt (đây là cuộc gọi API)
                        print(f"Đang gọi AI để tóm tắt logs cho student {student_id}...")
                        note = ai_summarizer.summarize_focus_logs(logs)
                        print(f"AI note: {note}")
                    else:
                        note = "Không có ghi nhận chi tiết." # Giữ nguyên nếu không có log

                except AttributeError as ae:
                        print(f"LỖI: focus_manager.py của bạn thiếu hàm 'get_student_full_logs'")
                        note = "Lỗi: focus_manager không có hàm get_student_full_logs."
                except Exception as e:
                    print(f"Lỗi khi gọi AI summarizer: {e}. Dùng note mặc định.")
                    note = f"Lỗi khi tóm tắt: {e}"
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                # >>> KẾT THÚC SỬA 2 <<<
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                print(f"Lỗi nghiêm trọng khi cập nhật focus_record cho student {student_id}: {e}")
                traceback.print_exc()
        
        print(f"Đã lưu kết quả session {session_to_finalize}.")
        messagebox.showinfo("Hoàn tất", f"Đã lưu kết quả session {session_to_finalize}.", parent=self.root)
